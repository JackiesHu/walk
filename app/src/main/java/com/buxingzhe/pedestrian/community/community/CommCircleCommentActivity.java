...
public class CommCircleCommentActivity extends BaseActivity {
    ...
    private void setData() {
        if (!TextUtils.isEmpty(walkRecordInfo.getLocation())){
            tv_address.setText(walkRecordInfo.getLocation());
        }
    ...
    public void onRightListener(View v) {
        ...
        paramsMap.put("walkRecord", walkRecordInfo.getId());
        ...
        NetRequestManager.getInstance().walkRecordComment(paramsMap,mSubscriber);
    }
    ...
    private void getExtr() {
        walkRecordInfo = getIntent().getParcelableExtra(WALKRECORDINFO);
    }
}
