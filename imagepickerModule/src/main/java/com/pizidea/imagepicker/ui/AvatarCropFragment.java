...
public class AvatarCropFragment extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        if (TextUtils.isEmpty(picPath)) {
            ...
            Picasso.with(mContext).load(new File(picPath)).into(superImageView);
        }
    ...
    public Bitmap getCropBitmap(int expectSize) {
        ...
        if (superImageView.getDrawable() != null) {
            if (((BitmapDrawable) superImageView.getDrawable()).getBitmap() != null) {
                srcBitmap = ((BitmapDrawable) superImageView.getDrawable()).getBitmap();
            }
        }
        if (srcBitmap == null) {
            Toast.makeText(mContext, "截图失败哦亲，请重试", Toast.LENGTH_SHORT).show();
            srcBitmap = BitmapFactory.decodeResource(getResources(), R.drawable.icon_default_avatar);
        }
